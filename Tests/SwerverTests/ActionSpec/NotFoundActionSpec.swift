    ...
    override func spec() {
        describe("A Not Found action") {
            ...
            it("can dispatch to the response builder to build a response of resource not found") {
                ...
                expect(notFoundResponse.statusCode).to(equal(StatusCode.notFound.rawValue))
                expect(notFoundResponse.statusPhrase).to(equal(StatusCode.notFound.getStatusPhrase()))
